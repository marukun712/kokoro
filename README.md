...
ポーズは `(u, v) => Transform` という関数 (`Pose`) で表現します。各頂点の UV 座標をウェイト計算に使うことで、「上半身ほど大きく動く」「毛先ほどよく揺れる」といった位置に連動した自然な変形を1つの関数で記述できます。複数のポーズはウェイト加算で合成され、`lerpPose` で線形補間もできます。
